...
class Profesor extends User implements iUser {
    public $tip = 'Profesor';
    use connectdb;
    ...
    public function info(){
        $sql="SELECT * from users where tip = 'Profesor'";
        $query = $this->db()->prepare($sql);
        $query->execute();
        $results = $query->fetchAll(PDO::FETCH_ASSOC);

        foreach($results as $v){
            echo "<b>Profesor </b> " . $v['ime']. " " . $v['prezime'] . ". <b>JMBG broj:</b> " . $v['jmbg'] . " <b>E-Mail adresa: </b>" . $v['email'] . "<br>";
        }
    }
}
?>